...
radon mi comment_classification -s >>.logs/maintainability.txt
export maintainability=$(radon mi comment_classification -s | grep -Po '(?<=\().*(?=\))' | perl -lane '$a+=$_ for(@F);$f+=scalar(@F);END{print $a/$f}' | xargs printf "%.1f%%\n")
